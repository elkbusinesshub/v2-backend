...
import {
  RepairBookingStatus,
  RepairPromoKind,
  type RepairService as RepairServiceRow,
} from '@prisma/client';
import {
  DomainException,
...
import {
  REPAIR_BOOKABLE_DAYS,
  REPAIR_CANCEL_CUTOFF_HOURS,
  REPAIR_CODE_MIN,
  REPAIR_CODE_SPAN,
  REPAIR_DEFAULT_LOCATION,
  REPAIR_TIME_SLOTS,
  REPAIR_UTC_OFFSET,
  REPAIR_VISIT_FEE,
...
import { toCategoryJson, toOfferJson, toRepairBookingJson, toServiceJson } from './repair.mapper';
...
const OFFSET_MS = -new Date(`1970-01-01T00:00:00${REPAIR_UTC_OFFSET}`).getTime();
const MS_PER_DAY = 24 * 60 * 60 * 1000;
const WEEKDAYS = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];

/** Fully resolved and validated cart — the single pricing source of truth. */
interface ResolvedQuote {
  lines: { service: RepairServiceRow; quantity: number; lineTotal: number }[];
  subtotal: number;
  visitFee: number;
...
@Injectable()
export class ElkRepService {
  private readonly logger = new Logger(ElkRepService.name);

  constructor(
    private readonly catalog: RepairCatalogRepository,
    private readonly bookings: RepairBookingsRepository,
  ...
  async getHomeFeed(user: AuthUser): Promise<Record<string, unknown>> {
    ...
    return {
      ...
      location: REPAIR_DEFAULT_LOCATION,
  ...
  async getBookingOptions(user: AuthUser): Promise<Record<string, unknown>> {
    ...
    return {
      ...
      timeSlots: [...REPAIR_TIME_SLOTS],
      visitFee: REPAIR_VISIT_FEE,
  ...
  async quote(dto: RepairQuoteDto): Promise<Record<string, unknown>> {
    return this.quoteJson(await this.resolveQuote(dto));
  }

  /**
   * Validates every cart line against the catalog and applies the app's
   * formula: subtotal = Σ price×qty; total = subtotal − discount + visit fee.
   */
  private async resolveQuote(dto: RepairQuoteDto): Promise<ResolvedQuote> {
    ...
    if (dto.promoCode) {
      ...
      discountAmount =
        promo.kind === RepairPromoKind.PERCENT
    ...
    return {
      ...
      promoCode,
      discountAmount,
      totalAmount: subtotal - discountAmount + REPAIR_VISIT_FEE,
    };
  }
  ...
  async createBooking(
    ...
    const booking = await this.bookings.create({
      booking: {
        code,
        userId: user.id,
        status: RepairBookingStatus.CONFIRMED,
    ...
    this.logger.log(`repair booking created: ${code} user=${user.id} lines=${q.lines.length}`);
    return toRepairBookingJson(booking);
  }

  async listBookings(user: AuthUser): Promise<Record<string, unknown>[]> {
    return (await this.bookings.listForUser(user.id)).map(toRepairBookingJson);
  }
  ...
  async cancelBooking(user: AuthUser, id: string): Promise<void> {
    ...
    const cutoff = new Date(Date.now() + REPAIR_CANCEL_CUTOFF_HOURS * 60 * 60 * 1000);
    const cancelled = await this.bookings.cancel(id, user.id, cutoff);
    if (!cancelled) {
      throw new DomainException(
        HttpStatus.CONFLICT,
        'NOT_CANCELLABLE',
        `Repairs can only be cancelled up to ${REPAIR_CANCEL_CUTOFF_HOURS}h before the arrival window`,
      );
    }
    this.logger.log(`repair booking cancelled (mock refund): ${booking.code}`);
  }

  /** Ops marks the technician's job done (admin-only until crew assignment exists). */
  async completeBooking(id: string): Promise<Record<string, unknown>> {
    ...
    this.logger.log(`repair booking completed: ${booking.code}`);
    return toRepairBookingJson((await this.bookings.findById(id))!);
  }

  // ─── management (admin) ────────────────────────────────────────────────────

  async createService(dto: CreateRepairServiceDto): Promise<Record<string, unknown>> {
  ...
  async updateService(id: string, dto: UpdateRepairServiceDto): Promise<Record<string, unknown>> {
  ...
  private resolveSlot(date: string, slot: string): Date {
    ...
    const scheduledAt = new Date(`${date}T${slot}:00.000${REPAIR_UTC_OFFSET}`);
  ...
  private async generateCode(): Promise<string> {
    for (let attempt = 0; attempt < 5; attempt++) {
      const code = `ELK-${REPAIR_CODE_MIN + randomInt(REPAIR_CODE_SPAN)}`;
  ...
  /** Next "PFX-NN" SKU in the category, e.g. AC-05. */
